...
const ReviewItem = ({ review, onDelete }: { review: Review, onDelete: () => void }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "users", review.userId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setUserData({
          name: data.name || review.user || 'User',
          avatar: data.avatar || review.avatar || null
        });
      }
    });
  ...
  return (
    <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex gap-4 items-start group">
      <div className="w-10 h-10 rounded-full overflow-hidden bg-zinc-200 dark:bg-zinc-700 flex-shrink-0 relative">
        {userData.avatar ? (
          <img 
            src={userData.avatar} 
      ...
      <div className="flex-1">
        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed italic">
          &quot;{review.comment}&quot;
        </p>
      </div>
    </div>
